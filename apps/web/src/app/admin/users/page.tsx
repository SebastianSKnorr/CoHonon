...
type User = {
  ...
  language: string | null; timezone: string | null; createdAt: string
}
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <span style={{ fontSize: '0.78rem', color: '#5a8099' }}>{users.length} total</span>
      </div>
      ...
      <div style={tableWrap}>
        ...
        {users.map((u, i) => (
          <div key={u.id} style={{ ...tdRow, borderBottom: i === users.length - 1 ? 'none' : '1px solid #162330' }}>
            <span style={td}>{u.name}</span>
            <span style={{ ...td, color: '#5a8099' }}>{u.email}</span>
            <span style={{ ...td }}>
              <span style={{ ...badge, background: u.role === 'ADMIN' ? 'rgba(90,180,212,0.12)' : 'transparent', color: u.role === 'ADMIN' ? '#5ab4d4' : '#5a8099' }}>
                {u.role}
              </span>
            </span>
...
const badge: React.CSSProperties = { fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', padding: '0.2rem 0.5rem', borderRadius: '3px', border: '1px solid #162330' }
